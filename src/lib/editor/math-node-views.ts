 ...
 *
 * Storage contract (from @moraya/core schema):
 ...
 *
 * These NodeViews REPLACE core's toDOM rendering (registered in setup.ts's
 * `nodeViews`). The NodeView DOM deliberately does NOT carry
 * `data-type="math_block"` so core's editor-props mousedown interceptor
 * (which exists to work around WebKit selection quirks on the toDOM
 * rendering) no longer applies — the NodeView owns interaction itself.
 */
